...
public class EditTrainController {
    ...
    public void setTrainData(ObservableList<String> trainData) {
        ...
        trainCodeField.setEditable(false); // Prevents PK modification issue
    ...
    private boolean isInputValid() {
        ...
        if (!ValidationUtils.isPositiveNumber(fairField.getText())) {
        ...
        if (!ValidationUtils.isValidTime(dep)) {
             errorMessage += "Departure Time must be HH:mm (e.g. 14:30)\n";
        }
        
        if (!ValidationUtils.isValidTime(arr)) {
